...
def server():
	...
	print "belum ada client tersambung"
	...
	while True:
		...
		for sock in ready_to_read:
			...
			if sock == server_socket: # cek kalau socketnya sesuai dengan socket server
				...
				broadcast(server_socket, sockfd, "(%s, %s) bergabung ke dalam chat room\n" % addr)
				...
				try:
					...
					if data:
						...
						if temp1[0]=="login" :
							...
							if login==0:
								send_msg(sock, "Silakhan login dahulu\n")
							...
							if login==0:
								send_msg(sock, "Silahkan login dahulu\n")
								...
								send_msg(sock, "Pengguna aktif: "+temp2+"\n")
...
def log_in (sock, user):
	...
	if s==1:
		...
		send_msg(sock, "Username salah, silahkan masukkan nama lain\n")
	else:
